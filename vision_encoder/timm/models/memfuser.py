...
class Memfuser(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.return_feature:
            ...
            traffic_feature = F.adaptive_avg_pool2d(traffic_feature_bev, (10, 10)).view(bs, -1, 100).permute(0, 2, 1)
            if self.return_bev:
                return torch.cat([traffic_feature, traffic_light_state_feature.view(bs, 1, -1), waypoints_feature], 1), traffic_feature_bev
            else:
...
@register_model
def memfuser_baseline_e1d3_return_feature_bev(**kwargs):
    model = Memfuser(
        enc_depth=1,
        dec_depth=3,
        embed_dim=256,
