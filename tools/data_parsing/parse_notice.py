...
def process_notice(line):
    ...
    Args
    ----
    line : str  # '<rel_path> <n_frames>'

    Returns
    -------
    list[dict] – processed samples (may be empty if errors occur)
    """
    ...
    try:
        ...
        for frame_id in range(frames):
            full_path = mf_dir / f"{frame_id:04d}.json"
        ...
        # ───────────────────────────────────────────────────────────────
        town_str = re.search(r'town(\d\d)', dir_path.as_posix()).group(1)
        town_id = int(town_str)
        weather_id = int(re.search(r'_w(\d+)_', dir_path.as_posix()).group(1))

        scenarios_path = Path(scenarios_dir) / f"town{town_id:02d}_all_scenarios.json"
        with scenarios_path.open('r') as file:
            scenarios_data = json.load(file)

        # ───────────────────────────────────────────────────────────────
        # 3) Apply notice processing rules
        # ───────────────────────────────────────────────────────────────
        processed_data = []
        for rule in notice_processing_rules:
            rule_obj = registered_class[rule]
            rule_id = rule_id_mapping_dict[rule]

            try:
                results = rule_obj.process(
                    {'data': json_data, 'town_id': town_id, 'weather_id': weather_id},
                    scenarios_data,
                    town_str
                )
                ...
                print(f"[rule-err] {rule} @ {path}: {rule_err}", file=sys.stderr)
        ...
        print(f"[route-err] {path}: {error_details}", file=sys.stderr)
        return []
