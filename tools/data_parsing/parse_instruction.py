...
def process(line):
    ...
    Args
    ----
    line : str       # one line from dataset_index.txt, '<rel_path> <n_frames>'

    Returns
    -------
    list[dict]  – processed samples (may be empty if the route is broken)
    """
    path, frames_s = line.split()
    dir_path  = Path(dataset_root) / path
    frames    = int(frames_s)

    try:
        # ───────────────────────────────────────────────────────────────
        #  1)  Load the measurement data (aggregate file if possible)
        # ───────────────────────────────────────────────────────────────
        json_data = []
        mf_dir = dir_path / "measurements_full"
        for fid in range(frames):
            fpath = mf_dir / f"{fid:04d}.json"
            with fpath.open("r") as fh:
                json_data.append(json.load(fh))

        # ───────────────────────────────────────────────────────────────
        #  2)  Extract route meta-data
        # ───────────────────────────────────────────────────────────────
        town_id    = int(re.search(r"town(\d\d)",   dir_path.as_posix()).group(1))
        weather_id = int(re.search(r"_w(\d+)_",     dir_path.as_posix()).group(1))

        # ───────────────────────────────────────────────────────────────
        #  3)  Apply all rules
        # ───────────────────────────────────────────────────────────────
        processed = []
        for rule_name in processing_rules:
            rule_obj  = registered_class[rule_name]
            rule_id   = rule_id_mapping_dict[rule_name]

            try:
                for res in rule_obj.process(
                        {"data": json_data,
                         "town_id": town_id,
                ...
                # log and continue with next rule
                print(f"[rule-err] {rule_name} @ {path}: {e}", file=sys.stderr)

        return processed

    except Exception as e:
        # Catch *any* unexpected problem with this route and move on.
        print(f"[route-err] {dir_path}: {e}", file=sys.stderr)
        return []
