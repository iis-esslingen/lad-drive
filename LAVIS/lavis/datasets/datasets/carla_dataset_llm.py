...
class CarlaVoiceDataset(BaseIODataset):
    ...
    def collater(batch):
        ...
        batch_out = default_collate(clean_batch)
    ...
    def __getitem__(self, idx):
        ...
        for frame_id in range(start_frame_id, end_frame_id, sample_interval):
            gt_command = measurements[frame_id]['gt_command']
            ...
            relative_theta = local_ego_theta - ego_theta
            ego_status = np.array([velocity, np.sin(relative_theta), np.cos(relative_theta)], dtype=np.float32)
        ...
        ego_statuses = self.pad_and_stack(ego_statuses)
        gt_commands = self.pad_and_stack(gt_commands)
        ...
        processed_data['ego_statuses'] = torch.from_numpy(ego_statuses).float()
        processed_data['gt_commands'] = torch.from_numpy(gt_commands).long()
        ...
        processed_data['nav_input'] = instruction_text
