...
class DriveTask(BaseTask):
    ...
    def evaluation(self, model, data_loader, cur_epoch=None, cuda_enabled=True):
        ...
        for samples in metric_logger.log_every(data_loader, print_freq, header):
            ...
            samples.update({"global_epoch": cur_epoch})
    ...
    def _train_inner_loop(
        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp, dtype=torch.bfloat16):#, dtype=torch.bfloat16):
