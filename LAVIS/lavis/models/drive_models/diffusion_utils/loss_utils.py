...
class TrajDecorrelation(nn.Module):
    def __init__(self, beta=0.02):
        super().__init__()
        self.beta = beta

    def calc_traj_decorrelation_loss(self, trajectories, eps=1e-8):
        B, num_modes, num_poses,_ = trajectories.shape 
        M = trajectories.flatten(1) # -2 for 31024
        M_norm = (M - M.mean(dim=0, keepdim=True)) / torch.sqrt(M.var(dim=0, keepdim=True) + eps)
        corr = M_norm.T @ M_norm
        off_diag_mask = ~torch.eye(M.shape[1], dtype=torch.bool, device=trajectories.device)
        corr_hat = corr[off_diag_mask]
        L_reg = (corr_hat ** 2).mean() / B
        return L_reg

    def forward(self, trajectories, base_loss):
        """
        trajectories: [B, num_modes, num_poses, 3]
        base_loss: original trajectory loss
        """
        decorr_loss = self.calc_traj_decorrelation_loss(trajectories)
        total_loss = base_loss + self.beta * decorr_loss
        return total_loss


class LossComputer(nn.Module):
    def __init__(self, cls_loss_weight=1.0, reg_loss_weight=1.0):
        super(LossComputer, self).__init__()
        self.cls_loss_weight = cls_loss_weight
        self.reg_loss_weight = reg_loss_weight
        #self.decorr_computer = TrajDecorrelation(beta=0.1)
    def forward(self, poses_reg, poses_cls, targets, plan_anchor):
        """
        pred_traj: (bs, 20, 5, 2)
        pred_cls: (bs, 20)
        plan_anchor: (bs,20, 5, 2)
        targets['trajectory']: (bs, 5, 2)
        """
        ...
        #ret_loss = self.decorr_computer(poses_reg, ret_loss)
        return ret_loss
