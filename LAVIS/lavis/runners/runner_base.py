...
class RunnerBase:
    ...
    def _load_checkpoint(self, url_or_filename):
        ...
        # Try loading optimizer and scaler only if they exist
        ...
        if "scaler" in checkpoint and hasattr(self, "scaler"):
            try:
                ...
                logging.warning(f"Could not load scaler state: {e}")

        # self.start_epoch = checkpoint["epoch"] + 1
        if "epoch" in checkpoint:
            self.start_epoch = checkpoint["epoch"] + 1
        else:
            self.start_epoch = 0  # Start from scratch when epoch is missing
